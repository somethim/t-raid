...
export const localIP = `ws://${getLocalIPAddress()}:8080`;
console.log(`WebSocket server running at: ${localIP}`);
        ...
        if (p.ws !== ws) {
          p.ws.send(
            JSON.stringify({
              type: "gameover",
              yourScore:
                match.scores[match.players.findIndex((p2) => p2.ws === p.ws)],
              opponentScore:
                match.scores[match.players.findIndex((p2) => p2.ws === ws)],
