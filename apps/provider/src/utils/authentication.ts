...
import { action, mutation, type MutationCtx, query, type QueryCtx } from "../definition/_generated/server";
...
export const authenticatedAction = customAction(action, {
  ...
  input: async (ctx, args) => {
    ...
    const user = await ctx.runQuery(internal.services.user.getById, { id: userId });
