...
export type SelectContentProps = EmptyObject;
...
export const Trigger = forwardRef<SelectTriggerRef, SlottablePressableProps>(
  ({ asChild, onPress: onPressProp, disabled = false, ...props }, ref) => {
    ...
    const onPress = (event: GestureResponderEvent) => {
      ...
      //TODO: fix wrong pageY
      augmentedRef.current?.measure((_x, _y, width, height, pageX, pageY) => {
        console.log("select trigger primitive pageY", pageY);
        setTriggerPosition({ width, pageX, pageY: pageY, height });
      });
 ...
 */
export const Portal = ({ forceMount, hostName, children }: PortalProps) => {
  const value = useRootContext();
...
>(
  (
    ...
    return (
      <Component ref={ref} onPress={onPress} {...props} className="z-[100]" />
    );
  },
);
