...
import { useInferredFormContext } from "./inferred-form";
...
const InferredFormControlTest = <S extends FieldShape, T extends z.ZodType>(
  { labelHidden, classList, ...props }: InferredFormControlProps<S, T>,
  ref: ForwardedRef<unknown>,
) => {
  const { fieldsVisibility } = useInferredFormContext();

  const fieldVisibility = fieldsVisibility?.[props.name] ?? true;

  const formValues = useWatch({
    disabled: typeof fieldVisibility !== "function",
  });

  const isFieldVisible =
    typeof fieldVisibility === "function"
      ? fieldVisibility(formValues)
      : fieldVisibility;
    ...
    switch (props.shape) {
      ...
      case "date": {
        const isDateRange =
          ...
          (!props.type && "shape" in props.constraint);
