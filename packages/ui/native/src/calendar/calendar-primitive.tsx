...
import { DAYS, MONTHS } from "@zenncore/utils/constants";
...
import { DrawerPressable } from "../drawer";
...
export const Calendar = ({
  ...
  return (
    <View className={cn("min-w-80 gap-2", className)}>
      {!headerHidden && (
        <CalendarHeader
          date={page}
          startDate={startDate}
...
export const CalendarHeader = ({
  ...
  return (
    <View className={cn("mb-2 flex-row items-end gap-2 px-2", className)}>
      <Select
        value={monthNames[normalizedDate.getMonth()]}
        onValueChange={(month) => {
          if (!month) return;

          const updatedMonthDate = new Date(normalizedDate);
      ...
      >
        <SelectTrigger variant={"underline"}>
          <Text className={"text-2xl capitalize"}>
            {monthNames[normalizedDate.getMonth()]}
          </Text>
        </SelectTrigger>
        <SelectContent insets={contentInsets}>
          <ScrollView className="flex-1">
            {months.map((targetMonth) => (
              <SelectItem key={targetMonth} value={targetMonth}>
      ...
      <Select
        ...
        onValueChange={(year) => {
          const updatedYearDate = new Date(normalizedDate);
          updatedYearDate.setFullYear(Number(year));
          onPageChange(updatedYearDate);
        }}
      >
        <SelectTrigger variant={"underline"}>
          <Text className={"text-2xl"}>{normalizedDate.getFullYear()}</Text>
        </SelectTrigger>
        <SelectContent>
          <ScrollView className="flex-1">
            {years.map((year) => (
      ...
      {showCalendarPageControls && (
        <>
          <Button
            ...
            onPress={() => onPageChange(subMonths(normalizedDate, 1))}
          >
          ...
          <Button
            ...
            onPress={() => onPageChange(addMonths(normalizedDate, 1))}
          >
...
export const CalendarBody = ({
  ...
  return (
    <View className={cn(className, "flex-col gap-2")}>
      ...
      <View className={"flex-row flex-wrap"}>
        {pageDays.map((pageDay) => {
          ...
          const isSingleSelectedDate =
            areEdgesSameDate && isSameDay(pageDay, startDate);
          ...
          return (
            <DrawerPressable
              ...
              onPress={() => onPick?.(startOfDay(pageDay))}
              className={cn(
                //transition-colors duration-300
                "aspect-square h-8 w-full max-w-[calc(100%/7.001)] items-center justify-center rounded-none bg-transparent",
                classList?.cell?.default,
                isSelectedDate &&
                  cn(
                    "bg-primary/10 active:bg-primary/40",
            ...
            >
              <Text
                className={cn(
                  "pointer-events-none text-xl tabular-nums transition-colors duration-300",
                  isDifferentMonth && "text-foreground-dimmed",
                  isDifferentMonth && isSelectedDate && "text-primary-rich",
                  isSelectedDate &&
                    cn(
                      "text-primary-rich",
                      // classList?.cell?.active,
