...
const getOTPInitValue = (
  ...
  return value && isValidOTPChar(value)
...
export const OTPInput = forwardRef(
  (
    ...
    const handleOTPInputChange = async (value: string, position: number) => {
      const isClipboardPastedValue =
        value.trim().length > 1 && otp[position] === "";
      const shouldOverwrite = position === 0 && value.trim().length >= length;
      ...
      if (shouldOverwrite) {
        handleOTPChange(value.trim(), 0);
        OTPInputFieldsRef.current[length - 1]?.focus();

        return;
      }

      if (isClipboardPastedValue) {
